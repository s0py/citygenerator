	...
	if z == 2:
		...
		with open("Cities.txt", "a") as text_file:
			...
			print('-----------------------------' +
				...
				'\n' + '-----------------------------' + '\n' + jobsPrintValue + '\n' + '-----------------------------' + 
				'\n' + 'The demographics of the City:' + 
				'\n' 'Humans {}%'.format(round(f0,1)), ' elfs {}%'.format(round(g0,1)),' Dwarves {}%'.format(round(h0,1)), ' Elves {}%'.format(round(i0,1)), "Kobolds {}%".format(round(j0,1)), '\n' + 'Humans:', int(round(f1)) ,' elfs:', int(round(g1)),' Dwarves:', int(round(h1)),' Elves:', int(round(i1,0)),' Kobolds:', int(round(j1,0)), '\n' + '\n', file=text_file)
			if iteration % 1 == 0:
				print( round((iteration + 1) / times * 100, 1), '%', barImage, end="\r")
